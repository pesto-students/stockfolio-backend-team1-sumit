...
def pastdata(symbol, start:date, end:date):
    if symbol is None:
        ...
        df = pd.DataFrame({"Symbol":Symbol,"Series":data.Series[0],"Open":data.Open[0],"High":data.High[0],"Low":data.Low[0],"Close":data.Close[0],"VWAP":data.VWAP[0],"Volume":data.Volume[0],"Turnover":data.Turnover[0],"Trades":data.Trades[0]}, index=[0])
        return(df.to_json())
