...
export default function createITcatalogue(){
    ...
    for (var symbols in symbolList){
      ...
      const stockDataResponse = fetch(`http://127.0.0.1:8000/stockdetails?symbol=${symbolList[symbols]}`)
            ...
            const indication = async(state)=>{
              ...
              const catalogueData = (`INSERT INTO "itcatalogue"("stock", "opening_bal", "closing_bal",
             ...
             var values = [json.info.symbol, json.priceInfo.open, json.priceInfo.lastPrice,
              Number(json.priceInfo.pChange).toFixed(2), cagr, state, json.industryInfo.sector, time, 1];
 ...
 function createDividentcatalogue(){

    var symbolList = ['NMDC', 'VEDL', 'RECLTD', 'ONGC', 'OIL', 'GAIL', 'PFC', 'OFSS', 'TIDEWATER', 'TATASTEEL'];
    for (var symbols in symbolList){
      ...
      const stockDataResponse = fetch(`http://127.0.0.1:8000/stockdetails?symbol=${symbolList[symbols]}`)
            ...
            const indication = async(state)=>{
              ...
              var time = (year + "-" + month + "-" + date + " " + hours + ":" + minutes + ":" + seconds);
              const catalogueData = (`INSERT INTO "dividentcatalogue"("stock", "opening_bal", "closing_bal",
              "pchange","cagr" , "status", "industry", "timestamp", "quantity") 
              VALUES($1, $2, $3, $4, $5, $6, $7, $8, $9)`)
